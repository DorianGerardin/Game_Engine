...
{
    // printf("Loading OBJ file %s...\n", stringPath);
    std::cout << "Loading OBJ file " << stringPath << "..." << std::endl;
    std::vector<unsigned int> vertexIndices, uvIndices, normalIndices;
    std::vector<glm::vec3> temp_vertices;
    std::vector<glm::vec2> temp_uvs;
    std::vector<glm::vec3> temp_normals;

    int stringSize = stringPath.length();
    char *path = (char *)malloc(sizeof(char) * (stringSize + 1));
    ...
    while (1)
    {
        ...
        else if (strcmp(lineHeader, "f") == 0)
        {
            ...
            vertexIndices.push_back(vertexIndex[1]);
            vertexIndices.push_back(vertexIndex[2]);
            // triangle = {(unsigned short)(vertexIndex[0] - 1), (unsigned short)(vertexIndex[1] - 1), (unsigned short)(vertexIndex[2] - 1)};
    ...
    for (unsigned int i = 0; i < vertexIndices.size(); i++)
    {
        ...
        // Put the attributes in buffers
        // out_vertices.push_back(vertex);
        out_indices.push_back(vertexIndices[i] - 1);
        // out_uvs.push_back(uv);
